...
func (c *Client) RPCVersion(ctx context.Context) (ok bool, serverVersion int64, serverMinimumVersion int64, err error) {
	...
	if payload.RPCVersion == 0 {
		err = errors.New("payload RPC Version is 0")
		return
	}
	if payload.RPCVersionMinimum == 0 {
		err = errors.New("payload RPC Version minimum is 0")
		return
	}
...
func (c *Client) SessionArgumentsSet(ctx context.Context, payload SessionArguments) (err error) {
	payload.BlocklistSize = 0
	payload.ConfigDir = ""
	payload.RPCVersion = 0
	payload.RPCVersionMinimum = 0
	payload.SessionID = ""
	payload.Version = ""
	// Exec
